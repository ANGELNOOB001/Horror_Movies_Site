// 無料のアクセスAPI (CountAPI) を使用する例
async function updateCounter() {
    // あなたのドメイン名に合わせてURLを書き換えてください
    const url = "https://api.countapi.xyz/hit/your-esther-archive.github.io/visits";

    try {
        const response = await fetch(url);
        const data = await response.json();
        // 6桁のゼロ埋め（000123のような表示）にする
        const formattedCount = String(data.value).padStart(6, '0');
        document.getElementById('count').innerText = formattedCount;
    } catch (error) {
