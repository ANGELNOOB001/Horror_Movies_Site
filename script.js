// angelnoob001さんのサイト専用のカウントを作成します
fetch('https://api.countapi.xyz/hit/angelnoob001-horror-site/visits')
    .then(res => res.json())
    .then(res => {
        const count = String(res.value).padStart(6, '0');
        document.getElementById('visitor-count').innerText = count;
    })
    .catch(err => {
        // エラー時はホラー演出として「機密アクセス」と表示
        document.getElementById('visitor-count').innerText = "000" + Math.floor(Math.random() * 999);
    });
